...
(function () {
  ...
  const WEDDING = new Date(CFG.dateISO || '2026-12-06T10:42:00+05:30');
  ...
  if (calBtn) {
    calBtn.addEventListener('click', (e) => {
      ...
      const ics = [
        'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//' + (CFG.icsProdId || 'Wedding') + '//Vivah//EN',
        'BEGIN:VEVENT', 'UID:' + (CFG.icsUid || 'shubh-vivah') + '@' + WEDDING.getFullYear(),
        'DTSTAMP:' + dt(new Date()),
        'DTSTART:' + dt(WEDDING), 'DTEND:' + dt(end),
        'SUMMARY:' + (CFG.summary || 'Shubh Vivah'),
        'DESCRIPTION:' + (CFG.description || 'With the blessings of our elders\\, join us for the muhurtham.'),
        'LOCATION:' + (CFG.location || ''),
        'END:VEVENT', 'END:VCALENDAR',
      ].join('\r\n');
      const url = URL.createObjectURL(new Blob([ics], { type: 'text/calendar' }));
      const a = document.createElement('a');
      a.href = url; a.download = (CFG.icsFilename || 'Shubh-Vivah') + '.ics';
  ...
  if (!reduce) {
    ...
    const svg = `<svg width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2c2 3 6 3 6 7 0 2-1 3-3 4 2 1 3 3 3 5 0-4-4-4-6-7-2 3-6 3-6 7 0-2 1-4 3-5-2-1-3-2-3-4 0-4 4-4 6-7z"/></svg>`;
